...
print("=" * 82)
print("  HEAD-TO-HEAD SOTA SSM COMPARISON SUITE (VECTORIZED & FAST)")
print("  Contenders: Mamba (S6 Selective SSM) vs LRU (ICML 2023) vs QU-SSM (Quasi-Unitary)")
print(f"  Execution Device: {DEVICE}")
print("=" * 82, flush=True)

# ==============================================================================
# 1. CONTENDER 1: MAMBA (S6 SELECTIVE SSM) - VECTORIZED
# Linear projections computed in parallel across sequence length L
# ==============================================================================
...
class MambaS6Model(nn.Module):
    def __init__(self, vocab_size=10, d_model=64, d_state=32):
        ...
        # HiPPO A initialization (Negative real diagonal)
    ...
    def forward(self, x):
        ...
        A = -torch.exp(self.A_log)       # [1, N]

        # VECTORIZED: Compute all projections at once in parallel
        ssm_params = self.x_proj(u)      # [B, L, 2N+1]
        dt_raw = ssm_params[:, :, :1]
        B_all = ssm_params[:, :, 1:self.d_state+1]
        C_all = ssm_params[:, :, self.d_state+1:]

        dt = F.softplus(self.dt_proj(dt_raw))  # [B, L, N]
        dA = torch.exp(dt * A.unsqueeze(1))    # [B, L, N] in (0, 1)
        dB = dt * B_all                        # [B, L, N]
        u_mean = u.mean(dim=-1, keepdim=True)  # [B, L, 1]
...
# ==============================================================================
# 2. CONTENDER 2: LRU (LINEAR RECURRENT UNIT - ICML 2023)
# ==============================================================================
...
class LRUModel(nn.Module):
    def __init__(self, vocab_size=10, d_model=64, d_state=32):
        ...
        # LRU radius r in (0, 1] and phase theta
...
# ==============================================================================
# 3. CONTENDER 3: QU-SSM (DYNAMIC SO(2) QUASI-UNITARY)
# ==============================================================================
...
class QUSSMContender(nn.Module):
    ...
    def forward(self, x):
        ...
        # VECTORIZED: Compute all dynamic rotations across L at once
        theta_dyn = self.theta_proj(emb) + self.theta_base  # [B, L, P]
        gamma_dyn = torch.sigmoid(self.gamma_proj(emb) + 4.0)  # [B, L, P]
...
# ==============================================================================
# 4. BENCHMARK RUNNER WITH LIVE PROGRESS LOGGING
# ==============================================================================
...
def run_benchmark_for(model_cls, name, delays=[500, 1000], steps=300):
    ...
    for d in delays:
        ...
        # Holdout evaluation on 200 unseen sequences
